...
const FinanceReview = () => {
    ...
    const loadData = async () => {
        ...
        try {
            ...
            const filtered = data.filter(c => c.department === 'Finance');
    ...
    const isLegalApproved = (contract) => contract?.reviews?.Legal?.status === 'Approved';

    const handleAction = async (id, action) => {
        const contract = contracts.find(c => c.id === id);
        if (!isLegalApproved(contract)) {
            setGateModal('Legal department has not yet approved this contract. You can add comments and review clauses, but cannot approve or reject until Legal completes their review.');
            return;
        }
        try {
            const status = action === 'Approve' ? 'Approved' : 'Rejected';
            await contractService.submitReview(id, 'Finance', status, comment || 'Action performed by Finance');
            await loadData();
        ...
        <div className={styles.container}>
            <header className={styles.pageHeader}>
                <div className={styles.titleArea}>
                    ...
                    <p>Manage and verify financial terms for active contract requests.</p>
                </div>
            </header>
            ...
            {contracts.length === 0 && !loading && (
                <div style={{ textAlign: 'center', padding: '60px', color: 'var(--text-muted)' }}>
                    All finance reviews are cleared.
                </div>
            )}
            ...
            {selectedContract && (
                <div style={{
                    ...
                    <div className={styles.reviewModal}>
                        ...
                        <div className={styles.actions}>
                            <button className={`${styles.btn} ${isLegalApproved(selectedContract) ? styles.approveBtn : styles.btnGated}`} onClick={() => handleAction(selectedContract.id, 'Approve')}>Approve Contract</button>
                            <button className={`${styles.btn} ${isLegalApproved(selectedContract) ? styles.rejectBtn : styles.btnGated}`} onClick={() => handleAction(selectedContract.id, 'Reject')}>Reject / Change</button>
                            <button
