...
const FinanceReview = ({ user }) => {
    ...
    const defaultClauses = [
        { id: 1, title: 'Payment Terms', content: 'Net 30 days upon receipt of valid invoice.', status: 'Pending' }
    ];
        ...
        try {
            ...
            const filtered = data.filter(c =>
                ...
                (
                    ...
                    (c.review_mode === 'parallel' && (c.review_stages || c.required_reviewers || []).includes('Finance'))
                ) &&
                c?.reviews?.Finance?.status !== 'Approved'
            );
    ...
    const isPrevApproved = (contract) => {
        ...
        const myIndex = stages.indexOf('Finance');
    ...
    const handleAction = async (id, action) => {
        ...
        try {
            ...
            const finalComment = `Finance Review: ${comment}`;

            await contractService.submitReview(id, 'Finance', status, finalComment);
            ...
            console.error('Finance review action failed', e);
...
const openReview = async (contract) => {
        ...
        const deptClauses = contractClauses.filter(c => {
            ...
            return CLAUSE_DEPT_MAP['Finance'].some(kw => text.includes(kw));
        });
        ...
        if (deptClauses.length > 0) {
            ...
            setClauses([{ 
                id: 'finance-combined', 
                title: 'Finance Provisions Summary', 
        ...
        <div className={styles.container}>
            <header className={styles.pageHeader}>
                 <div className={styles.titleArea}>
                     ...
                     <p>Manage and verify Finance terms for active contract requests.</p>
                 </div>
            </header>
                                        ...
                                        {changeRequests.length === 0 ? (
                                            ...
                                            changeRequests.map(cr => (
                                                <div key={cr._id || cr.id} style={{ padding: '10px', marginBottom: '8px', borderRadius: '8px', border: '0.5px solid var(--border-color)', background: 'rgba(255,255,255,0.02)' }}>
                                                    ...
                                                    {cr.status === 'Open' && cr.department === 'Finance' && (
