...
const ProcurementReview = ({ user }) => {
    ...
    const defaultClauses = [
        { id: 1, title: 'Vendor Terms', content: 'Vendor must maintain insurance of minimum $2M per incident.', status: 'Pending' }
    ];
        ...
        try {
            ...
            const filtered = data.filter(c =>
                ...
                (
                    ...
                    (c.review_mode === 'parallel' && (c.review_stages || c.required_reviewers || []).includes('Procurement'))
                ) &&
                c?.reviews?.Procurement?.status !== 'Approved'
            );
    ...
    const isPrevApproved = (contract) => {
        ...
        const myIndex = stages.indexOf('Procurement');
    ...
    const handleAction = async (id, action) => {
        ...
        try {
            ...
            const finalComment = `Procurement Review: ${comment}`;

            await contractService.submitReview(id, 'Procurement', status, finalComment);
            ...
            if (status === 'Approved') {
                setToast('✅ Approved. CAS generated and moved to DOA approval');
                setTimeout(() => setToast(null), 3000);
            ...
            console.error('Procurement review action failed', e);
...
const openReview = async (contract) => {
        ...
        const deptClauses = contractClauses.filter(c => {
            ...
            return CLAUSE_DEPT_MAP['Procurement'].some(kw => text.includes(kw));
        });
        
        if (deptClauses.length > 0) {
            const combinedContent = deptClauses.map(c => `[${c.type || 'Clause'}] ${c.text}`).join('\n\n');
            setClauses([{ 
                id: 'procurement-combined', 
                title: 'Procurement Provisions Summary', 
                                        ...
                                        {changeRequests.length === 0 ? (
                                            ...
                                            changeRequests.map(cr => (
                                                <div key={cr._id || cr.id} style={{ padding: '10px', marginBottom: '8px', borderRadius: '8px', border: '0.5px solid var(--border-color)', background: 'rgba(255,255,255,0.02)' }}>
                                                    ...
                                                    {cr.status === 'Open' && cr.department === 'Procurement' && (
