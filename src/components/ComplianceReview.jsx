...
const ComplianceReview = () => {
    ...
    const loadData = async () => {
        ...
        try {
            ...
            const filtered = data.filter(c => c.department === 'Compliance');
    ...
    const isFinanceApproved = (contract) => contract?.reviews?.Finance?.status === 'Approved';

    const handleAction = async (id, action) => {
        const contract = contracts.find(c => c.id === id);
        if (!isFinanceApproved(contract)) {
        ...
        try {
            ...
            await contractService.submitReview(id, 'Compliance', status, comment || 'Action performed by Compliance');
            await loadData();
            ...
            {loading ? (
                ...
                <div className={styles.reviewGrid}>
                    {contracts.map((c) => {
                        ...
                        const gated = !isFinanceApproved(c);
                        return (
            ...
            {selectedContract && (
                <div style={{
                    ...
                    <div className={styles.reviewModal}>
                        ...
                        <div className={styles.actions}>
                            <button className={`${styles.btn} ${isFinanceApproved(selectedContract) ? styles.approveBtn : styles.btnGated}`} onClick={() => handleAction(selectedContract.id, 'Approve')}>Approve Contract</button>
                            <button className={`${styles.btn} ${isFinanceApproved(selectedContract) ? styles.rejectBtn : styles.btnGated}`} onClick={() => handleAction(selectedContract.id, 'Reject')}>Reject / Change</button>
                            <button
